...
use App\Models\User;
use Modules\HRMSystem\Models\Leave;
...
class LeaveController extends AppBaseController
{
    public function index()
    {
        ...
        if(Auth::user()->can('manage leave'))
        {
            ...
            return view('hrmsystem::leave.index', compact('leaves'));
        }
        else
        {
    ...
    public function create()
    {
        if(Auth::user()->can('create leave'))
        {
            ...
            return view('hrmsystem::leave.create', compact('employees', 'leavetypes'));
        }
        else
        {
    ...
    public function store(Request $request)
    {
        ...
        if(Auth::user()->can('create leave'))
        {
            ...
            //$employee = User::where('user_id', '=', Auth::user()->id)->first();
            $leave_type = LeaveType::find($request->leave_type_id);
            $startDate = new \DateTime($request->start_date);
            $endDate = new \DateTime($request->end_date);
            $endDate->add(new \DateInterval('P1D'));
            $total_leave_days = !empty($startDate->diff($endDate)) ? $startDate->diff($endDate)->days : 0;
            if ($leave_type->duration >= $total_leave_days)
            {
                $leave    = new Leave();
                /* if(Auth::user()->type == "Employee")
                {
    ...
    public function edit(Leave $leave)
    {
        if(Auth::user()->can('edit leave'))
        {
            ...
            $leavetypes = LeaveType::get()->pluck('name', 'id');
            return view('hrmsystem::leave.edit', compact('leave', 'employees', 'leavetypes'));
        }
        else
        {
            return response()->json(['error' => __('Permission denied.')], 401);
    ...
    public function update(Request $request, $leave)
    {
        ...
        if(Auth::user()->can('edit leave'))
        {
            if($leave->created_by == Auth::user()->creatorId())
            {
                ...
                {
                    return redirect()->back()->with('error', __('Leave type ' . $leave_type->name . ' is provide maximum ' . $leave_type->duration . "  days please make sure your selected days is under " . $leave_type->duration . ' days.'));
                }
            }
            else
            {
    ...
    public function action($id)
    {
        ...
        return view('hrmsystem::leave.action', compact('employee', 'leavetype', 'leave'));
    }
    ...
    public function changeaction(Request $request)
    {
        ...
        if(!empty($employee->id))
        {
            if($setings['leave_status'] == 1)
            {
                ...
                //$employee     = User::where('id', $leave->employee_id)->first();
                $leave->name  = !empty($employee->first_name) ? $employee->first_name : '';
