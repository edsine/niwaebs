...
class AllowanceController extends AppBaseController
{
    ...
    public function allowanceCreate($id)
    {
        ...
        return view('hrmsystem::allowance.create', compact('employee', 'allowance_options','Allowancetypes'));
    }
    ...
    public function edit($allowance)
    {
        ...
        if(Auth::user()->can('edit allowance'))
        {
            if($allowance->created_by == Auth::user()->creatorId())
            {
                $allowance_options = AllowanceOption::where('created_by', Auth::user()->creatorId())->get()->pluck('name', 'id');
                $Allowancetypes =Allowance::$Allowancetype;
                return view('hrmsystem::allowance.edit', compact('allowance', 'allowance_options','Allowancetypes'));
            }
            else
            {
