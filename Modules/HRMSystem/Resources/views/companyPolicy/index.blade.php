            ...
            <div class="card-body table-border-style">
                    ...
                    <table class="table datatable">
                            ...
                            @foreach ($companyPolicy as $policy)
                                ...
                                @endphp
                                <tr>
                                    <td>{{ !empty($policy->branches)?$policy->branches->branch_name:'' }}</td>
                                    <td>{{ $policy->title }}</td>
                                    ...
                                    <td>
                                        @if (!empty($policy->attachment))
                                            <div class="action-btn bg-primary ms-2">
                                                ...
                                                <a  class="mx-3 btn1 btn-sm align-items-center" href="{{ $policyPath . '/' . $policy->attachment }}" download="">
                                                    <i class="fa fa-download text-white"></i>
                                                </a>
                                            </div>
                                            <div class="action-btn bg-secondary ms-2">
                                                <a class="mx-3 btn1 btn-sm align-items-center" href="{{ $policyPath . '/' . $policy->attachment }}" target="_blank"  >
                                                    <i class="fa fa-crosshairs text-white" data-bs-toggle="tooltip" data-bs-original-title="{{ __('Preview') }}"></i>
                                                </a>
                                            </div>
                                        @else
                                            <p>-</p>
                                    ...
                                    @if(Gate::check('edit company policy') || Gate::check('delete company policy'))
                                        <td>
                                            ...
                                            <div class="action-btn bg-primary ms-2">
                                                <a href="#" data-url="{{ route('company-policy.edit',$policy->id)}}" data-size="lg" data-ajax-popup="true" data-title="{{__('Edit Company Policy')}}" class="mx-3 btn1 btn-sm align-items-center" data-bs-toggle="tooltip" title="{{__('Edit')}}" data-original-title="{{__('Edit')}}"><i class="fa fa-pencil text-white"></i></a>
                                            </div>
                                                @endcan
                                                @can('edit company policy')
