...
use Modules\Accounting\Exports\InvoiceExport;
...
use Modules\Accounting\Models\InvoiceBankTransfer;
...
class InvoiceController extends AppBaseController
{
    ...
    public function create($customerId)
    {
        ...
        if(Auth::user()->can('create invoice'))
        {
            ...
            return view('accounting::invoice.create', compact('customers', 'invoice_number', 'product_services', 'category', 'customFields', 'customerId'));
        }
        else
        {
    ...
    public function customer(Request $request)
    {
        ...
        return view('accounting::invoice.customer_detail', compact('customer'));
    }
...
//                $invoiceProduct->discount    = isset($products[$i]['discount']) ? $products[$i]['discount'] : 0;
    ...
    public function edit($ids)
    {
        if(Auth::user()->can('edit invoice'))
        {
            ...
            return view('accounting::invoice.edit', compact('customers', 'product_services', 'invoice', 'invoice_number', 'category', 'customFields'));
        }
        else
        {
            return response()->json(['error' => __('Permission denied.')], 401);
    ...
    public function show($ids)
    {
        ...
        if(Auth::user()->can('show invoice'))
        {
            ...
            if(!empty($invoice->created_by) == Auth::user()->creatorId())
            {
                ...
                return view('accounting::invoice.view', compact('invoice', 'customer', 'iteams', 'invoicePayment', 'customFields', 'user'));
            }
            else
            {
                return redirect()->back()->with('error', __('Permission denied.'));
...
// If you need to check if the user has a specific role
    ...
    public function payment($invoice_id)
    {
        if(Auth::user()->can('create payment invoice'))
        {
           ...
           $customers = User::get()->map(function ($user) {
            ...
            return view('accounting::invoice.payment', compact('customers', 'categories', 'accounts', 'invoice'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    {
        return view('accounting::customer.invoice_send', compact('invoice_id'));
    }
    ...
    public function previewInvoice($template, $color)
    {
        ...
        return view('accounting::invoice.templates.' . $template, compact('invoice', 'preview', 'color', 'img', 'settings', 'customer', 'font_color', 'customFields'));
    }
    ...
    public function invoice($invoice_id)
    {
        ...
        if($invoice)
        {
            ...
            return view('accounting::invoice.templates.' . $settings['invoice_template'], compact('invoice', 'color', 'settings', 'customer', 'img', 'font_color', 'customFields'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function invoiceLink($invoiceId)
    {
        ...
        return view('accounting::invoice.customer_invoice', compact('settings','invoice', 'customer', 'iteams', 'invoicePayment', 'customFields', 'user','company_payment_setting'));
    }
