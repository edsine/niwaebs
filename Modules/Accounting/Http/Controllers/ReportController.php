...
use Modules\Accounting\Exports\AccountStatementExport;
use Modules\Accounting\Exports\PayrollExport;
use Modules\Accounting\Exports\ProductStockExport;
use Modules\Accounting\Exports\TrialBalancExport;
use Modules\Accounting\Exports\BalanceSheetExport;
use Modules\Accounting\Models\BankAccount;
use Modules\Accounting\Models\BillAccount;
use Modules\Accounting\Models\Bill;
use Modules\Accounting\Models\Branch;
use Modules\Accounting\Models\ClientDeal;
use Modules\Accounting\Models\Deal;
use Modules\Accounting\Models\Department;
use Modules\Accounting\Models\Employee;
use Modules\Accounting\Models\Lead;
use Modules\Accounting\Models\Leave;
use Modules\Accounting\Models\PaySlip;
use Modules\Accounting\Models\AttendanceEmployee;
...
use Modules\Accounting\Models\Pipeline;
...
use Modules\Accounting\Models\Source;
use Modules\Accounting\Models\StockReport;
use Modules\Accounting\Models\Transaction;
use App\Models\User;
use Modules\Accounting\Models\UserDeal;
use Modules\Accounting\Models\Utility;
use Modules\Accounting\Models\Tax;
use Modules\Accounting\Models\LeaveType;
use Modules\Accounting\Models\BankTransfer;
//use Modules\Accounting\Models\Vender;
use Modules\Accounting\Models\warehouse;
use Modules\Accounting\Models\WarehouseProduct;
use Carbon\Carbon;
...
class ReportController extends AppBaseController
{
    public function incomeSummary(Request $request)
    {
        if(Auth::user()->can('income report'))
        {
            ...
            $customer = User::where('created_by', '=', Auth::user()->creatorId())->get()->pluck('name', 'id');
    ...
    public function expenseSummary(Request $request)
    {
        if(Auth::user()->can('expense report'))
        {
            $account = BankAccount::where('created_by', '=', Auth::user()->creatorId())->get()->pluck('holder_name', 'id');
            $account->prepend('Select Account', '');
            $vender = User::where('created_by', '=', Auth::user()->creatorId())->get()->pluck('name', 'id');
    ...
    public function billSummary(Request $request)
    {
        if(Auth::user()->can('bill report'))
        {
            ...
            return view('accounting::report.bill_report', compact('bills', 'vender', 'status', 'totalBill', 'totalDueBill', 'totalPaidBill', 'billTotal', 'monthList', 'filter'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission Denied.'));
    ...
    public function accountStatement(Request $request)
    {
        if(Auth::user()->can('statement report'))
        {
            ...
            return view('accounting::report.statement_report', compact('reportData', 'account', 'types', 'filter'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission Denied.'));
    ...
    public function trialBalanceSummary(Request $request)
    {
        if(Auth::user()->can('trial balance report'))
        {
            ...
            $filter['startDateRange'] = $start;
            $filter['endDateRange']   = $end;
            return view('accounting::report.trial_balance', compact('filter', 'totalAccounts'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission Denied.'));
    ...
    public function leave(Request $request)
    {
        ...
        if(Auth::user()->can('manage report'))
        {
            ...
            return view('accounting::report.leave', compact('department', 'branch', 'leaves', 'filterYear', 'filter'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function employeeLeave(Request $request, $employee_id, $status, $type, $month, $year)
    {
        if(Auth::user()->can('manage report'))
        {
            ...
            return view('accounting::report.leaveShow', compact('leaves', 'leaveData'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function monthlyAttendance(Request $request)
    {
        if(Auth::user()->can('manage report'))
        {
            ...
            return view('accounting::report.monthlyAttendance', compact('employeesAttendance', 'branch', 'department', 'dates', 'data'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function payroll(Request $request)
    {
        ...
        if(Auth::user()->can('manage report'))
        {
            ...
            return view('accounting::report.payroll', compact('payslips', 'filterData', 'branch', 'department', 'filterYear'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function productStock(Request $request)
    {
        if(Auth::user()->can('stock report'))
        {
            ...
            return view('accounting::report.product_stock_report',compact('stocks'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission Denied.'));
    ...
    public function leadreport(Request $request)
    {
        ...
        return view('accounting::report.lead', compact('devicearray', 'leadsourceName', 'leadsourceeData', 'labels', 'data', 'filter', 'monthList','leads', 'leaduserName', 'leadusereData', 'user', 'leadpipelineName', 'leadpipelineeData'));
    }
    ...
    public function dealreport(Request $request)
    {
        ...
        $monthList = $month = $this->yearMonth();
        return view('accounting::report.deal', compact('devicearray', 'dealsourceName', 'dealsourceeData', 'dealUserData', 'dealUserName', 'dealpipelineName', 'dealpipelineeData', 'data', 'labels', 'dealClientName', 'dealClientData','monthList'));
    }
    ...
    public function warehouseReport()
    {
        ...
        return view('accounting::report.warehouse',compact('warehouse','totalWarehouse','totalProduct','warehouseProductData','warehousename'));
...
//        dd($request->all());
        ...
        $vendor     = User::where('created_by', Auth::user()->creatorId())->get()->pluck('name', 'id');
        ...
        $vendors = User::where('id', '=', $request->vendor)->where('created_by', Auth::user()->creatorId())->first();
        $filter['vendor']       = !empty($vendors)?$vendors->name:'';

        return view('accounting::report.daily_purchase',compact('warehouse','vendor','arrDuration','data','filter'));
    }
    ...
    public function purchaseMonthlyReport(Request $request)
    {
        ...
        return view('accounting::report.monthly_purchase',compact('monthList','yearList','warehouse','vendor','arrDuration','data','filter'));
    }
...
//        dd($request->all());
        ...
        $warehouses = warehouse::where('id', '=', $request->warehouse)->where('created_by', Auth::user()->creatorId())->first();
        $filter['warehouse']       = !empty($warehouses)?$warehouses->name:'';
        $customers = User::where('id', '=', $request->customer)->where('created_by', Auth::user()->creatorId())->first();
        $filter['customer']       = !empty($customers)?$customers->name:'';

        return view('accounting::report.daily_pos',compact('warehouse','customer','arrDuration','data','filter'));
    }
    ...
    public function posMonthlyReport(Request $request)
    {
        ...
        return view('accounting::report.monthly_pos',compact('monthList','yearList','warehouse','customer','arrDuration','data','filter'));
    }
...
//            -----------------------------
            ...
            return view('accounting::report.pos_vs_purchase', compact('filter'),  $data);
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    ->groupBy('month', 'year', 'invoice_id', 'category_id', 'id')
    ->get();
...
//            -----------------------------
            ...
            return view('accounting::report.profit_loss', compact('filter'),  $data);
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function monthlyCashflow(Request $request)
    {
        if(Auth::user()->can('loss & profit report'))
        {
            ...
            return view('accounting::report.monthly_cashflow', compact('filter'), $data);
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function quarterlyCashflow(Request $request)
    {
        ...
        if(Auth::user()->can('loss & profit report'))
        {
            ...
            return view('accounting::report.quarterly_cashflow', compact('filter'), $data);
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
            ...
            foreach($types as $type)
            {
                ...
                foreach($subTypes as $subType)
                {
                    ...
                    foreach($accounts as $account)
                    {
                        ...
                        $getAccount = \Modules\Accounting\Models\ChartOfAccount::where('name', $account->name)->first();
    ...
    public function balanceSheetPrint(Request $request)
    {
        if(Auth::user()->can('bill report'))
        {
            ...
            return view('accounting::report.balance_sheet_receipt', compact('filter', 'chartAccounts'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission Denied.'));
    ...
    public function trialBalancePrint(Request $request)
    {
        if(Auth::user()->can('trial balance report'))
        {
            ...
            return view('accounting::report.trial_balance_receipt', compact('filter', 'totalAccounts'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission Denied.'));
