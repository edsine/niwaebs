...
use Modules\Accounting\Exports\BillExport;
...
class BillController extends AppBaseController
{
    ...
    public function create($vendorId)
    {
        ...
        if(Auth::user()->can('create bill'))
        {
            ...
            return view('accounting::bill.create', compact('venders', 'bill_number', 'product_services', 'category', 'customFields', 'vendorId','chartAccounts'));
        }
        else
        {
    ...
    public function show($ids)
    {
        ...
        if(Auth::user()->can('show bill'))
        {
            ...
            if(!empty($bill) && $bill->created_by == Auth::user()->creatorId())
            {
                ...
                return view('accounting::bill.view', compact('bill', 'vendor', 'items', 'billPayment', 'customFields'));
            }
            else
            {
                return redirect()->back()->with('error', __('Permission denied.'));
...
//                dd($items);
                ...
                return view('accounting::bill.edit', compact('venders', 'product_services', 'bill', 'bill_number', 'category',
    ...
    public function sent($id)
    {
        if(Auth::user()->can('send bill'))
        {
            ...
            $vender = User::where('id', $bill->vender_id)->first();
...
//            Utility::userBalance('vendor', $bill->vender_id, $payment->amount, 'credit');
    ...
    public function venderBill(Request $request)
    {
        if(Auth::user()->can('manage vender bill'))
        {
            ...
            return view('accounting::bill.index', compact('bills', 'status'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission Denied.'));
    ...
    public function venderBillShow($id)
    {
        if(Auth::user()->can('show bill'))
        {
            ...
            if($bill->created_by == Auth::user()->creatorId())
            {
                ...
                return view('accounting::bill.view', compact('bill', 'vendor', 'iteams'));
            }
            else
            {
                return redirect()->back()->with('error', __('Permission denied.'));
    ...
    {
        $vender = User::where('id', '=', $request->id)->first();

        return view('accounting::bill.vender_detail', compact('vender'));
    }


    public function venderBillSend($bill_id)
    {
        return view('accounting::vender.bill_send', compact('bill_id'));
    }
...
//            Mail::to($email)->send(new VenderBillSend($bill));
    ...
    public function previewBill($template, $color)
    {
        ...
        return view('accounting::bill.templates.' . $template, compact('bill', 'preview', 'color', 'img', 'settings', 'vendor', 'font_color', 'customFields'));
    }
    ...
    public function bill($bill_id)
    {
        ...
        if($bill)
        {
            ...
            return view('accounting::bill.templates.' . $settings['bill_template'], compact('bill', 'color', 'settings', 'vendor', 'img', 'font_color', 'customFields'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function invoiceLink($billId)
    {
        ...
        $bill->customField = CustomField::getData($bill, 'bill');
         ...
         return view('accounting::bill.customer_bill', compact('bill', 'vendor', 'iteams', 'customFields','billPayment','user'));
    }
