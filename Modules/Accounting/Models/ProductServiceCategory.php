...
class ProductServiceCategory extends Model
{
    ...
    {
        return $this->hasMany('Modules\Accounting\Models\Revenue', 'category_id', 'id');
    }

    public function incomeCategoryRevenueAmount()
    {
        $year    = date('Y');
        $revenue = $this->hasMany('Modules\Accounting\Models\Revenue', 'category_id', 'id')->where('created_by', Auth::user()->creatorId())->whereRAW('YEAR(date) =?', [$year])->sum('amount');

        $invoices     = $this->hasMany('Modules\Accounting\Models\Invoice', 'category_id', 'id')->where('created_by', Auth::user()->creatorId())->whereRAW('YEAR(send_date) =?', [$year])->get();
        $invoiceArray = array();
    ...
    {
        $year    = date('Y');
        $payment = $this->hasMany('Modules\Accounting\Models\Payment', 'category_id', 'id')->where('created_by', Auth::user()->creatorId())->whereRAW('YEAR(date) =?', [$year])->sum('amount');

        $bills     = $this->hasMany('Modules\Accounting\Models\Bill', 'category_id', 'id')->where('created_by', Auth::user()->creatorId())->whereRAW('YEAR(send_date) =?', [$year])->get();
        $billArray = array();
