                                ...
                                <tr>
                                    ...
                                    @foreach($monthList as $month)
                                        ...
                                        <td>{{\Auth::user()->priceFormat($incomeArr[$productService->id][$month])}}
                                            <p>{{($budget['income_data'][$productService->id][$month] !=0)? (\Modules\Accounting\Models\Budget::percentage($budget['income_data'][$productService->id][$month],$incomeArr[$productService->id][$month])!=0) ? '('.(\Modules\Accounting\Models\Budget::percentage($budget['income_data'][$productService->id][$month],$incomeArr[$productService->id][$month]).'%)') :'':''}}</p>
                                        </td>
                                        <td>{{\Auth::user()->priceFormat($overBudgetAmount)}}
                                            <p class="{{($budget['income_data'][$productService->id][$month] < $overBudgetAmount)? 'green-text':''}} {{($budget['income_data'][$productService->id][$month] > $overBudgetAmount)? 'red-text':''}}" >{{($budget['income_data'][$productService->id][$month] !=0)? (\Modules\Accounting\Models\Budget::percentage($budget['income_data'][$productService->id][$month],$overBudgetAmount) !=0) ?'('.(\Modules\Accounting\Models\Budget::percentage($budget['income_data'][$productService->id][$month],$overBudgetAmount).'%)') :'':''}}</p>
                                ...
                                @if(!empty($budgetTotal) )
                                    @foreach($monthList as $month)
                                        ...
                                        <td class="text-dark"><strong>{{\Auth::user()->priceFormat($incomeTotalArr[$month])}}</strong>
                                            <p>{{($budgetTotal[$month] !=0)? (\Modules\Accounting\Models\Budget::percentage($budgetTotal[$month],$incomeTotalArr[$month])!=0) ? '('.(\Modules\Accounting\Models\Budget::percentage($budgetTotal[$month],$incomeTotalArr[$month]).'%)') :'':''}}</p>

                                        </td>
                                        <td class="text-dark"><strong>{{\Auth::user()->priceFormat($overBudgetTotalArr[$month]) }}</strong>
                                            <p class="{{($budgetTotal[$month] < $overBudgetAmount)? 'green-text':''}} {{($budgetTotal[$month] > $overBudgetAmount)? 'red-text':''}}">{{($budgetTotal[$month] !=0)? (\Modules\Accounting\Models\Budget::percentage($budgetTotal[$month],$overBudgetTotalArr[$month])!=0) ?'('.(\Modules\Accounting\Models\Budget::percentage($budgetTotal[$month],$overBudgetTotalArr[$month]).'%)') :'':''}}</p>
                                ...
                                <tr>
                                    ...
                                    @foreach($monthList as $month)
                                        ...
                                        <td>{{\Auth::user()->priceFormat($expenseArr[$productService->id][$month])}}
                                            <p>{{($budget['expense_data'][$productService->id][$month] !=0)? (\Modules\Accounting\Models\Budget::percentage($budget['expense_data'][$productService->id][$month],$expenseArr[$productService->id][$month])!=0) ? '('.(\Modules\Accounting\Models\Budget::percentage($budget['expense_data'][$productService->id][$month],$expenseArr[$productService->id][$month]).'%)') :'':''}}</p>

                                        </td>
                                        <td>{{\Auth::user()->priceFormat($overBudgetAmount)}}
                                            <p class="{{($budget['expense_data'][$productService->id][$month] < $overBudgetAmount)? 'green-text':''}} {{($budget['expense_data'][$productService->id][$month] > $overBudgetAmount)? 'red-text':''}}" >{{($budget['expense_data'][$productService->id][$month] !=0)? (\Modules\Accounting\Models\Budget::percentage($budget['expense_data'][$productService->id][$month],$overBudgetAmount) !=0) ?'('.(\Modules\Accounting\Models\Budget::percentage($budget['expense_data'][$productService->id][$month],$overBudgetAmount).'%)') :'':''}}</p>
                                ...
                                @if(!empty($budgetExpenseTotal) )
                                    @foreach($monthList as $month)
                                        ...
                                        <td class="text-dark"><strong>{{\Auth::user()->priceFormat($expenseTotalArr[$month])}}</strong>
                                            <p>{{($budgetExpenseTotal[$month] !=0)? (\Modules\Accounting\Models\Budget::percentage($budgetExpenseTotal[$month],$expenseTotalArr[$month])!=0) ? '('.(\Modules\Accounting\Models\Budget::percentage($budgetExpenseTotal[$month],$expenseTotalArr[$month]).'%)') :'':''}}</p>
                                        </td>
                                        <td class="text-dark"><strong>{{\Auth::user()->priceFormat($overExpenseBudgetTotalArr[$month]) }}</strong>
                                            <p class="{{($budgetExpenseTotal[$month] < $overExpenseBudgetTotalArr[$month])? 'green-text':''}} {{($budgetExpenseTotal[$month] >$overExpenseBudgetTotalArr[$month])? 'red-text':''}}">{{($budgetExpenseTotal[$month] !=0)? (\Modules\Accounting\Models\Budget::percentage($budgetExpenseTotal[$month],$overExpenseBudgetTotalArr[$month])!=0) ? '('.(\Modules\Accounting\Models\Budget::percentage($budgetExpenseTotal[$month],$overExpenseBudgetTotalArr[$month]).'%)') :'':''}}</p>
                                ...
                                @if(!empty($budgetprofit) )
                                    ...
                                    @foreach($monthList as $month)
                                        ...
                                        <td class="text-dark"><strong>{{\Auth::user()->priceFormat($actualprofit[$month]) }}</strong>
                                            <p>{{($budgetprofit[$month] !=0)? (\Modules\Accounting\Models\Budget::percentage($budgetprofit[$month],$actualprofit[$month])!=0) ?'('.(\Modules\Accounting\Models\Budget::percentage($budgetprofit[$month],$actualprofit[$month]).'%)') :'':''}}</p>

                                        </td>
                                        <td class="text-dark"><strong>{{\Auth::user()->priceFormat($overbudgetprofit[$month]) }}</strong>
                                            <p class="{{($budgetprofit[$month] < $overbudgetprofit[$month])? 'green-text':''}} {{($budgetprofit[$month] < $overbudgetprofit[$month])? 'green-text':''}}">{{($budgetprofit[$month] !=0)? (\Modules\Accounting\Models\Budget::percentage($budgetprofit[$month],$overbudgetprofit[$month])!=0) ? '('.(\Modules\Accounting\Models\Budget::percentage($budgetprofit[$month],$overbudgetprofit[$month]).'%)') :'':''}}</p>
                            ...
                            @foreach ($incomeproduct as $productService)
                                <tr>
                                    ...
                                    @foreach($quarterly_monthlist as $month)
                                        ...
                                        <td>{{\Auth::user()->priceFormat($overBudgetAmount)}}
                                            <p class="{{($budget['income_data'][$productService->id][$month] < $overBudgetAmount)? 'green-text':''}} {{($budget['income_data'][$productService->id][$month] > $overBudgetAmount)? 'red-text':''}}">{{($budget['income_data'][$productService->id][$month] !=0)? '('.(\Modules\Accounting\Models\Budget::percentage($budget['income_data'][$productService->id][$month],$overBudgetAmount).'%)') :''}}</p>
                                        </td>
                            ...
                            <tr class="total">
                                ...
                                @if(!empty($budgetTotal) )
                                    ...
                                    @foreach($quarterly_monthlist as $month)
                                        ...
                                        <td class="text-dark"><strong>{{\Auth::user()->priceFormat($overBudgetTotalArr[$month]) }}</strong>
                                            <p class="{{($budgetTotal[$month] < $overBudgetTotalArr[$month])? 'green-text':''}} {{($budgetTotal[$month] > $overBudgetTotalArr[$month])? 'red-text':''}}">{{($budgetTotal[$month] !=0)? '('.(\Modules\Accounting\Models\Budget::percentage($budgetTotal[$month],$overBudgetTotalArr[$month]).'%)') :''}}</p>
                                        </td>
                                    @endforeach
                                @endif
                            ...
                            <tr class="total">
                                ...
                                @if(!empty($budgetprofit) )
                                    @foreach($quarterly_monthlist as $month)
                                        ...
                                        <td class="text-dark"><strong>{{\Auth::user()->priceFormat($overbudgetprofit[$month]) }}</strong>
                                            <p class="{{($budgetprofit[$month] < $overbudgetprofit[$month])? 'green-text':''}} {{($budgetprofit[$month] > $overbudgetprofit[$month])? 'red-text':''}}">{{($budgetprofit[$month] !=0)? (\Modules\Accounting\Models\Budget::percentage($budgetprofit[$month],$overbudgetprofit[$month])!=0) ? '('.(\Modules\Accounting\Models\Budget::percentage($budgetprofit[$month],$overbudgetprofit[$month]).'%)') :'':''}}</p>
                                        </td>
                                    @endforeach
                                @endif
                            ...
                            <tr class="total">
                                ...
                                @if(!empty($budgetTotal) )
                                    @foreach($half_yearly_monthlist as $month)
                                        ...
                                        <td class="text-dark"><strong>{{\Auth::user()->priceFormat($overBudgetTotalArr[$month]) }}</strong>
                                            <p class="{{($budgetTotal[$month] < $overBudgetTotalArr[$month])? 'green-text':''}} {{($budgetTotal[$month] > $overBudgetTotalArr[$month])? 'red-text':''}}">{{($budgetTotal[$month] !=0)? (\Modules\Accounting\Models\Budget::percentage($budgetTotal[$month],$overBudgetTotalArr[$month])!=0) ?'('.(\Modules\Accounting\Models\Budget::percentage($budgetTotal[$month],$overBudgetTotalArr[$month]).'%)') :'':''}}</p>
