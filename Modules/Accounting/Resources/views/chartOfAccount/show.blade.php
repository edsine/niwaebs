                        ...
                        <div class="table-responsive">
                            <table class="table">
                                ...
                                @php
                                    ...
                                    $totalDebit   = 0;
                                    $totalCredit  = 0;
                                    $chartDatas =Modules\Accounting\Models\Utility::getAccountData($account->id,$filter['startDateRange'],$filter['endDateRange']);
                                    $accountName = \Modules\Accounting\Models\ChartOfAccount::find($account->id);
                                @endphp
                                ...
                                @foreach($chartDatas['invoice'] as $invoiceData)
                                    <tr>
                                        ...
                                        @php
                                            $invoice = \Modules\Accounting\Models\Invoice::where('id',$invoiceData->invoice_id)->first();
                                        @endphp
                                ...
                                @foreach($chartDatas['invoicepayment'] as $invoicePaymentData)
                                    <tr>
                                        <td>{{$accountName->name}}</td>
                                        @php
                                            $invoice = \Modules\Accounting\Models\Invoice::where('id',$invoicePaymentData->invoice_id)->first();
                                        @endphp
                                        <td>{{ (!empty($invoice->customer)?$invoice->customer->name:'-')}}</td>
                                ...
                                @foreach($chartDatas['bill'] as $billProduct)
                                    <tr>
                                        ...
                                        $bill = \Modules\Accounting\Models\Bill::find($billProduct->bill_id);
                                        /* $vendor = \App\Models\Vender::find(!empty($bill)?$bill->vender_id:''); */
                                        $vendor = \App\Models\User::find(!empty($bill)?$bill->vender_id:'');
                                        @endphp
                                        <td>{{!empty($vendor)?$vendor->first_name : '-'}}</td>
                                ...
                                @foreach($chartDatas['billdata'] as $billData)
                                    @php
                                        $bill = \Modules\Accounting\Models\Bill::find($billData->ref_id);
                                        $vendor = \App\Models\User::find(!empty($bill)?$bill->vender_id:'');
                                    @endphp
                                    <tr>
                                        <td>{{$accountName->name}}</td>
                                ...
                                @foreach($chartDatas['billpayment'] as $billPaymentData)
                                    @php
                                        $bill = \Modules\Accounting\Models\BillPayment::where('bill_id',$billPaymentData->bill_id)->first();
                                        $billId= \Modules\Accounting\Models\Bill::find($billPaymentData->bill_id);
                                        $vendor = \App\Models\User::find($billId->vender_id);
                                    @endphp
                                    <tr>
                                        <td>{{$accountName->name}}</td>
                                        <td>{{!empty($vendor)?$vendor->first_name : '-'}}</td>
                                ...
                                @foreach($chartDatas['payment'] as $paymentData)
                                    ...
                                    @php
                                        $vendor = \App\Models\User::find($paymentData->vender_id);
                                    @endphp
                                    <tr>
                                        <td>{{$accountName->name}}</td>
