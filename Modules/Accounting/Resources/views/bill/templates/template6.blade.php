@php
    $settings_data = \Modules\Accounting\Models\Utility::settingsById($bill->created_by);
    ...
    <div class="bill-body">
            ...
            <tr >
                <td >
                    <table class="no-space" >
                        ...
                        <tr>
                            <td >{{__('Number')}}:</td>
                            <td class="text-right">{{\Modules\Accounting\Models\Utility::billNumberFormat($settings,$bill->bill_id)}}</td>
                        </tr>
                        <tr>
                            <td>{{__('Bill Date')}}:</td>
                            <td class="text-right">{{\Modules\Accounting\Models\Utility::dateFormat($settings,$bill->issue_date)}}</td>
                        </tr>

                        <tr>
                            <td>{{__('Due Date')}}:</td>
                            <td class="text-right">{{\Modules\Accounting\Models\Utility::dateFormat($settings,$bill->due_date)}}</td>
                        </tr>
        ...
        <table class="add-border bill-summary" style="margin-top: 30px;">
            ...
            <tr style="border-bottom:1px solid {{ $color }};">
                ...
                <td>{{\Modules\Accounting\Models\Utility::priceFormat($settings,$bill->totalRate)}}</td>
                <td>{{\Modules\Accounting\Models\Utility::priceFormat($settings,$bill->totalDiscount)}}</td>
                <td>{{\Modules\Accounting\Models\Utility::priceFormat($settings,$bill->totalTaxPrice) }}</td>
                <td>{{\Modules\Accounting\Models\Utility::priceFormat($settings,$bill->getSubTotal())}}</td>
            </tr>
            <tr>
                <td colspan="4"></td>
                <td colspan="2" class="sub-total">
                    <table class="total-table">
                        ...
                        @if($bill->getTotalDiscount())
                            <tr style="border-bottom:1px solid {{ $color }};">
                                <td>{{__('Discount')}}:</td>
                                <td>{{\Modules\Accounting\Models\Utility::priceFormat($settings,$bill->getTotalDiscount())}}</td>
                            </tr>
                        @endif
                        ...
                        <tr style="border-bottom:1px solid {{ $color }};">
                            <td>{{__('Total')}}:</td>
                            <td>{{\Modules\Accounting\Models\Utility::priceFormat($settings,$bill->getSubTotal()-$bill->getTotalDiscount()+$bill->getTotalTax())}}</td>
                        </tr>
                        <tr style="border-bottom:1px solid {{ $color }};">
                            <td>{{__('Paid')}}:</td>
                            <td>{{\Modules\Accounting\Models\Utility::priceFormat($settings,($bill->getTotal()-$bill->getDue())-($bill->billTotalDebitNote()))}}</td>
                        </tr>
                        <tr style="border-bottom:1px solid {{ $color }};">
                            <td>{{__('Debit Note')}}:</td>
                            <td>{{\Modules\Accounting\Models\Utility::priceFormat($settings,($bill->billTotalDebitNote()))}}</td>
                        </tr>
                        <tr style="border-bottom:1px solid {{ $color }};">
                            <td>{{__('Due Amount')}}:</td>
                            <td>{{\Modules\Accounting\Models\Utility::priceFormat($settings,$bill->getDue())}}</td>
                        </tr>
...
@if(!isset($preview))
    @include('accounting::bill.script');
@endif
