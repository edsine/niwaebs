                        ...
                        <div class="card-body">
                            <div class="row timeline-wrapper">
                                <div class="col-md-6 col-lg-4 col-xl-4">
                                    ...
                                    <p class="text-muted text-sm mb-3"><i class="fa fa-clock mr-2"></i>{{__('Created on ')}}{{\Auth::user()->dateFormat($bill->bill_date)}}</p>
                                    @can('edit bill')
                                        <a href="{{ route('bill.edit',\Crypt::encrypt($bill->id)) }}" class="btn btn-sm btn-primary" data-bs-toggle="tooltip" data-original-title="{{__('Edit')}}"><i class="fa fa-pencil mr-2"></i>{{__('Edit')}}</a>
                                ...
                                <div class="col-md-6 col-lg-4 col-xl-4">
                                    ...
                                    <p class="text-muted text-sm mb-3">
                                        @if($bill->status!=0)
                                            <i class="fa fa-clock mr-2"></i>{{__('Sent on')}} {{\Auth::user()->dateFormat($bill->send_date)}}
                                        @else
                                            @can('send bill')
                                    ...
                                    @if($bill->status==0)
                                        @can('send bill')
                                            <a href="{{ route('bill.sent',$bill->id) }}" class="btn btn-sm btn-warning" data-bs-toggle="tooltip" data-original-title="{{__('Mark Sent')}}"><i class="fa fa-send mr-2"></i>{{__('Send')}}</a>
                                        @endcan
                                    @endif
                                </div>
                                <div class="col-md-6 col-lg-4 col-xl-4">
                                    ...
                                    @if($bill->status!=0)
                                        @can('create payment bill')
                                            <a href="#" data-url="{{ route('bill.payment',$bill->id) }}" data-ajax-popup="true" data-title="{{__('Add Payment')}}" class="btn btn-sm btn-info" data-original-title="{{__('Add Payment')}}"><i class="fa fa-report-money mr-2"></i>{{__('Add Payment')}}</a> <br>
                                        @endcan
                                    @endif
                            ...
                            <div class="row mt-3">
                                <div class="col">
                                    <small>
                                        ...
                                        @if($bill->status == 0)
                                            <span class="badge bg-primary p-2 px-3 rounded">{{ __(\Modules\Accounting\Models\Bill::$statues[$bill->status]) }}</span>
                                        @elseif($bill->status == 1)
                                            <span class="badge bg-warning p-2 px-3 rounded">{{ __(\Modules\Accounting\Models\Bill::$statues[$bill->status]) }}</span>
                                        @elseif($bill->status == 2)
                                            <span class="badge bg-danger p-2 px-3 rounded">{{ __(\Modules\Accounting\Models\Bill::$statues[$bill->status]) }}</span>
                                        @elseif($bill->status == 3)
                                            <span class="badge bg-info p-2 px-3 rounded">{{ __(\Modules\Accounting\Models\Bill::$statues[$bill->status]) }}</span>
                                    ...
                                    <div class="table-responsive mt-3">
                                        <table class="table mb-0 table-striped">
                                            ...
                                            @foreach($items as $key =>$item)
                                                ...
                                                @if(!empty($item->tax))
                                                    @php
                                                        $taxes=\Modules\Accounting\Models\Utility::tax($item->tax);
                                                        $totalQuantity+=$item->quantity;
                                                        $totalRate+=$item->price;
                                                        $totalDiscount+=$item->discount;
                                                        foreach($taxes as $taxe){
                                                            $taxDataPrice=\Modules\Accounting\Models\Utility::taxRate($taxe->rate,$item->price,$item->quantity,$item->discount);
                                                ...
                                                @if(!empty($item->product_id))
                                                    <tr>
                                                            ...
                                                            @if(!empty($item->tax))
                                                                <table>
                                                                    ...
                                                                    @foreach($taxes as $tax)
                                                                        ...
                                                                        @php
                                                                            $taxPrice=\Modules\Accounting\Models\Utility::taxRate($tax->rate,$item->price,$item->quantity,$item->discount) ;
                                                        ...
                                                        @php
                                                            $chartAccount = \Modules\Accounting\Models\ChartOfAccount::find($item->chart_account_id);
                                                        @endphp
