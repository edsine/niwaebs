            ...
            $('#flaterwave-payment-form').ajaxForm(function (res) {
                ...
                if (res.flag == 1) {
                    ...
                    var x = getpaidSetup({
                        ...
                        currency: '{{Modules\Accounting\Models\Utility::getValByName('site_currency')}}',
                    ...
                    <div class="card-body">
                        <div class="row timeline-wrapper">
                            <div class="col-md-6 col-lg-4 col-xl-4">
                                ...
                                <p class="text-muted text-sm mb-3"><i class="fa fa-clock mr-2"></i>{{__('Created on ')}}{{\Auth::user()->dateFormat($invoice->issue_date)}}</p>
                                @can('edit invoice')
                                    <a href="{{ route('invoice.edit',\Crypt::encrypt($invoice->id)) }}" class="btn btn-sm btn-primary" data-bs-toggle="tooltip" data-original-title="{{__('Edit')}}"><i class="fa fa-pencil mr-2"></i>{{__('Edit')}}</a>
                                @endcan
                            </div>
                            <div class="col-md-6 col-lg-4 col-xl-4">
                                <div class="timeline-icons"><span class="timeline-dots"></span>
                                    <i class="fa fa-mail text-warning"></i>
                                </div>
                                <h6 class="text-warning my-3">{{__('Send Invoice')}}</h6>
                                <p class="text-muted text-sm mb-3">
                                    @if($invoice->status!=0)
                                        <i class="fa fa-clock mr-2"></i>{{__('Sent on')}} {{\Auth::user()->dateFormat($invoice->send_date)}}
                                    @else
                                        @can('send invoice')
                                ...
                                @if($invoice->status==0)
                                    @can('send bill')
                                        <a href="{{ route('invoice.sent',$invoice->id) }}" class="btn btn-sm btn-warning" data-bs-toggle="tooltip" data-original-title="{{__('Mark Sent')}}"><i class="fa fa-send mr-2"></i>{{__('Send')}}</a>
                                    @endcan
                                @endif
                            </div>
                            <div class="col-md-6 col-lg-4 col-xl-4">
                                <div class="timeline-icons"><span class="timeline-dots"></span>
                                    <i class="fa fa-report-money text-info"></i>
                                </div>
                                ...
                                @if($invoice->status!=0)
                                    @can('create payment invoice')
                                        <a href="#" data-url="{{ route('invoice.payment',$invoice->id) }}" data-ajax-popup="true" data-title="{{__('Add Payment')}}" class="btn btn-sm btn-info" data-original-title="{{__('Add Payment')}}"><i class="fa fa-report-money mr-2"></i>{{__('Add Payment')}}</a> <br>
                                    @endcan
                                @endif
                    ...
                    <div class="invoice">
                        <div class="invoice-print">
                            ...
                            <div class="row">
                                ...
                                @endif
                                @if(Modules\Accounting\Models\Utility::getValByName('shipping_display')=='on')
                                    <div class="col ">
                                        <small>
                            ...
                            <div class="row mt-3">
                                <div class="col">
                                    <small>
                                        ...
                                        @if($invoice->status == 0)
                                            <span class="badge bg-primary">{{ __(\Modules\Accounting\Models\Invoice::$statues[$invoice->status]) }}</span>
                                        @elseif($invoice->status == 1)
                                            <span class="badge bg-warning">{{ __(\Modules\Accounting\Models\Invoice::$statues[$invoice->status]) }}</span>
                                        @elseif($invoice->status == 2)
                                            <span class="badge bg-danger">{{ __(\Modules\Accounting\Models\Invoice::$statues[$invoice->status]) }}</span>
                                        @elseif($invoice->status == 3)
                                            <span class="badge bg-info">{{ __(\Modules\Accounting\Models\Invoice::$statues[$invoice->status]) }}</span>
                                        @elseif($invoice->status == 4)
                                            <span class="badge bg-success">{{ __(\Modules\Accounting\Models\Invoice::$statues[$invoice->status]) }}</span>
                                        @endif
                                    </small>
                                </div>
                                    ...
                                    <div class="table-responsive mt-2">
                                        <table class="table mb-0 table-striped">
                                            ...
                                            @foreach($iteams as $key =>$iteam)
                                                @if(!empty($iteam->tax))
                                                    @php
                                                        $taxes=Modules\Accounting\Models\Utility::tax($iteam->tax);
                                                        $totalQuantity+=$iteam->quantity;
                                                        $totalRate+=$iteam->price;
                                                        $totalDiscount+=$iteam->discount;
                                                        foreach($taxes as $taxe){
                                                            $taxDataPrice=Modules\Accounting\Models\Utility::taxRate($taxe->rate,$iteam->price,$iteam->quantity,$iteam->discount);
                                                        ...
                                                        @if(!empty($iteam->tax))
                                                            <table>
                                                                ...
                                                                @foreach($taxes as $tax)
                                                                    @php
                                                                        $taxPrice=Modules\Accounting\Models\Utility::taxRate($tax->rate,$iteam->price,$iteam->quantity,$iteam->discount) ;
                    ...
                    <div class="table-responsive">
                        <table class="table ">
                            ...
                            @if(!empty($invoice->payments) && $invoice->bankPayments)
                                @php
                                    $path =\Modules\Accounting\Models\Utility::get_file('uploads/order');
                                @endphp
                                    ...
                                    <tr>
                                        <td>
                                            @if(!empty($payment->add_receipt))
                                                <a href="{{asset(Storage::url('uploads/payment')).'/'.$payment->add_receipt}}" download="" class="btn btn-sm btn-secondary btn-icon rounded-pill" target="_blank"><span class="btn-inner--icon"><i class="fa fa-download"></i></span></a>
                                            @else
                                                -
                                            @endif
                                        </td>
...
{{--                                                                <span class="input-group-prepend"><span class="input-group-text">{{ Modules\Accounting\Models\Utility::getValByName('site_currency') }}</span></span>--}}
...
{{--                                                                    <a href="#" class="apply-btn apply-coupon" data-toggle="tooltip" data-title="{{__('Apply')}}"><i class="fa fa-save"></i></a>--}}
{{--                                                                </div>--}}
{{--                                                            </div>--}}
{{--                                                        </div>--}}
{{--                                                        <div class="form-group mt-3">--}}
