@php
    $settings_data = \Modules\Accounting\Models\Utility::settingsById($invoice->created_by);
            ...
            <tr>
                <td >
                    ...
                    <table class="no-space">
                        ...
                        <tr >
                            <td>{{__('Number')}}:</td>
                            <td class="text-right">{{\Modules\Accounting\Models\Utility::invoiceNumberFormat($settings,$invoice->invoice_id)}}</td>
                        </tr>
                        <tr>
                            <td>{{__('Issue Date')}}:</td>
                            <td class="text-right">{{\Modules\Accounting\Models\Utility::dateFormat($settings,$invoice->issue_date)}}</td>
                        </tr>
                        <tr>
                            <td>{{__('Due Date:')}}</td>
                            <td class="text-right">{{\Modules\Accounting\Models\Utility::dateFormat($settings,$invoice->due_date)}}</td>
                        </tr>
    ...
    <div class="invoice-body">
        ...
        <table class=" invoice-summary" style="margin-top: 30px;">
            ...
            <tr style="border-bottom:1px solid {{ $color }};">
                ...
                <td>{{\Modules\Accounting\Models\Utility::priceFormat($settings,$invoice->totalRate)}}</td>
                <td>{{\Modules\Accounting\Models\Utility::priceFormat($settings,$invoice->totalDiscount)}}</td>
                <td>{{\Modules\Accounting\Models\Utility::priceFormat($settings,$invoice->totalTaxPrice) }}</td>
                <td>{{\Modules\Accounting\Models\Utility::priceFormat($settings,$invoice->getSubTotal())}}</td>
            </tr>
            <tr>
                <td colspan="4"></td>
                <td colspan="2" class="sub-total">
                    <table class="total-table">
                        ...
                        @if($invoice->getTotalDiscount())
                            <tr style="border-bottom:1px solid {{ $color }};">
                                <td>{{__('Discount')}}:</td>
                                <td>{{\Modules\Accounting\Models\Utility::priceFormat($settings,$invoice->getTotalDiscount())}}</td>
                            </tr>
                        @endif
                        ...
                        <tr style="border-bottom:1px solid {{ $color }};">
                            <td>{{__('Total')}}:</td>
                            <td>{{\Modules\Accounting\Models\Utility::priceFormat($settings,$invoice->getSubTotal()-$invoice->getTotalDiscount()+$invoice->getTotalTax())}}</td>
                        </tr>
                        <tr style="border-bottom:1px solid {{ $color }};">
                            <td>{{__('Paid')}}:</td>
                            <td>{{\Modules\Accounting\Models\Utility::priceFormat($settings,($invoice->getTotal()-$invoice->getDue())-($invoice->invoiceTotalCreditNote()))}}</td>
                        </tr>
                        <tr style="border-bottom:1px solid {{ $color }};">
                            <td>{{__('Credit Note')}}:</td>
                            <td>{{\Modules\Accounting\Models\Utility::priceFormat($settings,($invoice->invoiceTotalCreditNote()))}}</td>
                        </tr>
                        <tr style="border-bottom:1px solid {{ $color }};">
                            <td>{{__('Due Amount')}}:</td>
                            <td>{{\Modules\Accounting\Models\Utility::priceFormat($settings,$invoice->getDue())}}</td>
                        </tr>
...
@if(!isset($preview))
    @include('accounting::invoice.script');
@endif
