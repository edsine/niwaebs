   ...
   // $logo=asset(Storage::url('uploads/logo/'));
    $logo=\Modulels\Accounting\Models\Utility::get_file('uploads/logo');
    $company_favicon=\Modulels\Accounting\Models\Utility::companyData($invoice->created_by,'company_favicon');
    $setting = \DB::table('settings')->where('created_by', $user->creatorId())->pluck('value','name')->toArray();
    $settings_data = \Modulels\Accounting\Models\Utility::settingsById($invoice->created_by);
    $color = (!empty($setting['color'])) ? $setting['color'] : 'theme-3';
    $company_setting=\Modulels\Accounting\Models\Utility::settingsById($invoice->created_by);

    $getseo= \Modulels\Accounting\Models\Utility::getSeoSetting();
    $metatitle =  isset($getseo['meta_title']) ? $getseo['meta_title'] :'';
    $metsdesc= isset($getseo['meta_desc'])?$getseo['meta_desc']:'';
    $meta_image = \Modulels\Accounting\Models\Utility::get_file('uploads/meta/');
    $meta_logo = isset($getseo['meta_image'])?$getseo['meta_image']:'';
    $get_cookie = \Modulels\Accounting\Models\Utility::getCookieSetting();
@endphp
<!DOCTYPE html>
                  ...
                  <div class="invoice-print">
                    ...
                    <div class="row">
                        <div class="col text-end">
                            ...
                            <div class="me-4">
                                <small>
                                  ...
                                  {{\Modulels\Accounting\Models\Utility::dateFormat($settings,$invoice->issue_date)}}<br><br>
                                </small>
                            </div>
                              <small>
                                  <strong>{{__('Due Date')}} :</strong><br>
                                  {{\Modulels\Accounting\Models\Utility::dateFormat($settings,$invoice->due_date)}}<br><br>
                              </small>
                      ...
                      <div class="row">
                          ...
                          @endif
                          @if(\Modulels\Accounting\Models\Utility::companyData($invoice->created_by,'shipping_display')=='on')
                              <div class="col">
                                  <small>
                      ...
                      <div class="row mt-3">
                          <div class="col">
                              <small>
                                  ...
                                  @if($invoice->status == 0)
                                      <span class="badge bg-primary">{{ __(\Modulels\Accounting\Models\Invoice::$statues[$invoice->status]) }}</span>
                                  @elseif($invoice->status == 1)
                                      <span class="badge bg-warning">{{ __(\Modulels\Accounting\Models\Invoice::$statues[$invoice->status]) }}</span>
                                  @elseif($invoice->status == 2)
                                      <span class="badge bg-danger">{{ __(\Modulels\Accounting\Models\Invoice::$statues[$invoice->status]) }}</span>
                                  @elseif($invoice->status == 3)
                                      <span class="badge bg-info">{{ __(\Modulels\Accounting\Models\Invoice::$statues[$invoice->status]) }}</span>
                                  @elseif($invoice->status == 4)
                                      <span class="badge bg-success">{{ __(\Modulels\Accounting\Models\Invoice::$statues[$invoice->status]) }}</span>
                                  @endif
                              </small>
                          </div>
                              ...
                              <div class="table-responsive mt-2">
                                  <table class="table mb-0 table-striped">
                                      ...
                                      @foreach($iteams as $key =>$iteam)
                                          @if(!empty($iteam->tax))
                                              @php
                                                  $taxes=\Modulels\Accounting\Models\Utility::tax($iteam->tax);
                                                  $totalQuantity+=$iteam->quantity;
                                                  $totalRate+=$iteam->price;
                                                  $totalDiscount+=$iteam->discount;
                                                  foreach($taxes as $taxe){
                                                      $taxDataPrice=\Modulels\Accounting\Models\Utility::taxRate($taxe->rate,$iteam->price,$iteam->quantity,$iteam->discount);
                                          ...
                                          <tr>
                                              ...
                                              <td>{{\Modulels\Accounting\Models\Utility::priceFormat($settings,$iteam->price)}}</td>
                                              <td>{{\Modulels\Accounting\Models\Utility::priceFormat($settings,$iteam->discount)}}</td>
                                              <td>
                                                  ...
                                                  @if(!empty($iteam->tax))
                                                      <table>
                                                          ...
                                                          @foreach($taxes as $tax)
                                                              ...
                                                              @php
                                                                  $taxPrice=\Modulels\Accounting\Models\Utility::taxRate($tax->rate,$iteam->price,$iteam->quantity,$iteam->discount) ;
                                                                  $totalTaxPrice+=$taxPrice;
                                                              @endphp
                                                              <tr>
                                                                  <td>{{$tax->name .' ('.$tax->rate .'%)'}}</td>
                                                                  <td>{{\Modulels\Accounting\Models\Utility::priceFormat($settings,$taxPrice)}}</td>
                                                              </tr>
                                                          @endforeach
                                                      </table>
                                                  @else
                                              ...
                                              <td class="text-end">{{\Modulels\Accounting\Models\Utility::priceFormat($settings,($iteam->price * $iteam->quantity - $iteam->discount) + $totalTaxPrice)}}</td>
                                          </tr>
                                      @endforeach
                                      <tfoot>
                                      <tr>
                                          ...
                                          <td><b>{{\Modulels\Accounting\Models\Utility::priceFormat($settings,$totalRate)}}</b></td>
                                          <td><b>{{\Modulels\Accounting\Models\Utility::priceFormat($settings,$totalDiscount)}}</b></td>
                                          <td><b>{{\Modulels\Accounting\Models\Utility::priceFormat($settings,$totalTaxPrice)}}</b></td>
                                          <td></td>
                                      </tr>
                                      <tr>
                                          <td colspan="6"></td>
                                          <td class="text-end"><b>{{__('Sub Total')}}</b></td>
                                          <td class="text-end">{{\Modulels\Accounting\Models\Utility::priceFormat($settings,$invoice->getSubTotal())}}</td>
                                      </tr>

                                          <tr>
                                              <td colspan="6"></td>
                                              <td class="text-end"><b>{{__('Discount')}}</b></td>
                                              <td class="text-end">{{\Modulels\Accounting\Models\Utility::priceFormat($settings,$invoice->getTotalDiscount())}}</td>
                                          </tr>
                                      ...
                                      @if(!empty($taxesData))
                                          @foreach($taxesData as $taxName => $taxPrice)
                                              <tr>
                                                  ...
                                                  <td class="text-end">{{ \Modulels\Accounting\Models\Utility::priceFormat($settings,$taxPrice) }}</td>
                                              </tr>
                                          @endforeach
                                      @endif
                                      <tr>
                                          ...
                                          <td class="blue-text text-end">{{\Modulels\Accounting\Models\Utility::priceFormat($settings,$invoice->getTotal())}}</td>
                                      </tr>
                                      <tr>
                                          <td colspan="6"></td>
                                          <td class="text-end"><b>{{__('Paid')}}</b></td>
                                          <td class="text-end">{{\Modulels\Accounting\Models\Utility::priceFormat($settings,($invoice->getTotal()-$invoice->getDue())-($invoice->invoiceTotalCreditNote()))}}</td>
                                      </tr>
                                      <tr>
                                          <td colspan="6"></td>
                                          <td class="text-end"><b>{{__('Credit Note')}}</b></td>
                                          <td class="text-end">{{\Modulels\Accounting\Models\Utility::priceFormat($settings,($invoice->invoiceTotalCreditNote()))}}</td>
                                      </tr>
                                      <tr>
                                          <td colspan="6"></td>
                                          <td class="text-end"><b>{{__('Due')}}</b></td>
                                          <td class="text-end">{{\Modulels\Accounting\Models\Utility::priceFormat($settings,$invoice->getDue())}}</td>
                                      </tr>
                                      </tfoot>
                                  </table>
                              </div>
            ...
            <div class="card-body table-border-style">
                <div class="table-responsive">
                    <table class="table">
                        ...
                        @php
                            $path =\Modulels\Accounting\Models\Utility::get_file('uploads/order');
                        @endphp
                        ...
                        @if(!empty($invoice->payments) && $invoice->bankPayments)
                            @foreach($invoice->payments as $key =>$payment)
                                ...
                                <tr>
                                    <td>{{\Modulels\Accounting\Models\Utility::dateFormat($settings,$payment->date)}}</td>
                                    <td>{{\Modulels\Accounting\Models\Utility::priceFormat($settings,$payment->amount)}}</td>
                                    ...
                                    <td>
                                        @if(!empty($payment->receipt))
                                            <a href="{{ $path . '/' . $payment->receipt }}" target="_blank">
                                                <i class="fa fa-file"></i>{{__('Receipt')}}</a>
                            ...
                            @foreach($invoice->bankPayments as $key =>$bankPayment)
                                <tr>
                                    <td>{{\Modulels\Accounting\Models\Utility::dateFormat($settings,$bankPayment->date)}}</td>
                                    <td>{{\Modulels\Accounting\Models\Utility::priceFormat($settings,$bankPayment->amount)}}</td>
                                    ...
                                    <td>
                                        @if(!empty($bankPayment->receipt))
                                            <a href="{{ $path . '/' . $bankPayment->receipt }}" target="_blank">
                                                <i class="fa fa-file"></i> {{__('Receipt')}}
                                            </a>
                                        @endif
