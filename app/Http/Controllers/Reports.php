...
class Reports extends Controller
{
    ...
    public function getassetactivityreport(){
        ...
        ->select('asset_history.*', 'niwa_assets.name as asset', 'users.first_name as employees', 'asset_type.name as type', 'location.name as location')
        ->leftJoin('niwa_assets', 'niwa_assets.id', '=', 'asset_history.assetid')
        ->leftJoin('asset_type', 'niwa_assets.typeid', '=', 'asset_type.id')
        ->leftJoin('users', 'users.id', '=', 'asset_history.employeeid')
        ->leftJoin('location', 'location.id', '=', 'niwa_assets.locationid')
    ...
    public function getcomponentactivityreport(){
        ...
        ->select('component_assets.*', 'component.name as component','niwa_assets.name as asset', 'asset_type.name as type', 'location.name as location')
        ->leftJoin('niwa_assets', 'niwa_assets.id', '=', 'component_assets.assetid')
        ->leftJoin('asset_type', 'niwa_assets.typeid', '=', 'asset_type.id')
        ->leftJoin('location', 'location.id', '=', 'niwa_assets.locationid')
    ...
    public function getdatabytypereport(Request $request){
    	$data = DB::table('niwa_assets')
		->leftJoin('brand', 'niwa_assets.brandid', '=', 'brand.id')
		->leftJoin('supplier', 'niwa_assets.supplierid', '=', 'supplier.id')
		->leftjoin('asset_type', 'niwa_assets.typeid', '=', 'asset_type.id')
		->leftJoin('location', 'niwa_assets.locationid', '=', 'location.id')
		->select(['niwa_assets.*', 'supplier.name as supplier', 'brand.name as brand','brand.id as brandid', 'asset_type.name as type' , 'location.name as location']);
       ...
       return Datatables::of($data)
        ...
        ->filter(function ($query) use ($request) {
				if ($request->has('assettype')) {
					...
					$query->where('niwa_assets.typeid', 'like', "%{$request->get('assettype')}%");
				} 
			})
        ->rawColumns(['pictures'])
        ->make(true);		
       ...
       return Datatables::of($data)
        ...
        ->filter(function ($query) use ($request) {
				if ($request->has('statustype')) {
					...
					$query->where('niwa_assets.status', 'like', "%{$request->get('statustype')}%");
				} 
			})
       ...
       return Datatables::of($data)
        ...
        ->filter(function ($query) use ($request) {
				if ($request->has('supplierid')) {
					...
					$query->where('niwa_assets.supplierid', 'like', "%{$request->get('supplierid')}%");
				} 
			})
        ->rawColumns(['pictures'])
        ->make(true);		
       ...
       return Datatables::of($data)
        ...
        ->filter(function ($query) use ($request) {
				if ($request->has('locationid')) {
					...
					$query->where('niwa_assets.locationid', 'like', "%{$request->get('locationid')}%");
				} 
			})
        ->rawColumns(['pictures'])
        ->make(true);		
